    ...
    Column(
        ...
        Box(Modifier.fillMaxSize()) {
            if (state.marathonRace?.coordinates?.isNotEmpty() == true) {
                AnimateCamera(state = state, cameraPosition = cameraPosition, coordinate = state.startLocation)
...
fun LoadMap(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
            GoogleMap(
                ...
                state.marathonRace?.let { Polyline(points = it.coordinates, color = Color.Red) }
